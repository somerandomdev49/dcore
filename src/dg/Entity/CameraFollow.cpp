...
namespace dg::entity
{
	...
	void CameraFollowComponent::Start(dcore::world::EntityHandle self)
	{
		...
		if(Transform_ == nullptr) LOG_F(ERROR, "CameraFollowComponent's entity has to have a TransformComponent!");
...
#define CAMERA_MOVE_SPEED    0.05f
			if(Character_->IsMoving())
			{
				...
				case 3:
					...
					[[fallthrough]];
				case 0:
					if(Timer_ >= CAMERA_FREELOOK_WAIT) CameraMode_ = 1;
					else
						Timer_ += dcore::event::TimeManager::Instance()->GetDeltaTime();
					break;
				case 1:
